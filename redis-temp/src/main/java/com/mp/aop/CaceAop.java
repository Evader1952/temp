...
public class CaceAop {
     ...
     **/
    @Pointcut("@within(com.mp.annotation.CaceAOP)")
...
//        String key = className + ":" + methodName + ":" + Arrays.toString(strings) + ":" + arg;
        String key = className + ":" + methodName + ":" + arg;
//        //判断是否增加阅读
//        if (methodName.indexOf("read") != -1) {
...
//                    return Result.ok(map.get("data"));
        ...
        } else if ((methodName.indexOf("add") != -1) || (methodName.indexOf("del") != -1)
                || (methodName.indexOf("update") != -1) || methodName.indexOf("publishAdop") != -1) {
            Set<String> keys = redisTemplate.keys(className + "*");
            redisTemplate.delete(keys);
